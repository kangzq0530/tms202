...
public class SkillData {
    ...
    private static void loadSkillsFromWz(){
        String wzDir = CoreConfig.WZ_PATH + "/Skill.wz";
        ...
        for (File file : files) {
            ...
            for (Node mainNode : nodes) {
                ...
                Node skillChild = XMLApi.getFirstChildByNameBF(mainNode, "skills");
                for(Node skillNode : XMLApi.getAllChildren(skillChild)) {
                    ...
                    int skillId;
                    if(StringUtils.isNumber(skillIdName)) {
    ...
    public static void loadMobSkillsFromWz() {
        String wzDir = CoreConfig.WZ_PATH + "/Skill.wz/MobSkill.img.xml";
                    ...
                    switch(name) {
                        ...
                        default:
                            if(!unks.contains(name)) {
                                log.warn(String.format("Unkown MobSkillStat %s with value %s (skills %d level %d)", name, value, skillID, level));
                                unks.add(name);
                            }
                            break;
                    }
    ...
    private static MobSkillInfo loadMobSkillInfoByIdAndLevel(short id, short level) {
        File file = new File(String.format("%s/mobSkills/%d-%d.dat", CoreConfig.DAT_PATH, id, level));
        if(file.exists()) {
            return loadMobSkillFromFile(file);
        } else {
            log.error(String.format("Could not load mob skills %d (level %d)!", id, level));
            return null;
        }
    }
    ...
    public static void generateDatFiles() {
        loadMobSkillsFromWz();
        saveMobSkillsToDat(CoreConfig.DAT_PATH + "/mobSkills");
    }
