...
public class MobData {
    ...
    public static void generateDatFiles() {
        ...
        saveToFile(CoreConfig.DAT_PATH + "/mobs");
    }
...
//        return getMobs().stream().filter(mob -> mob.getTemplateId() == id).findFirst().orElse(loadMobFromFile(id));
    ...
    private static Mob loadMobFromFile(int id) {
        File file = new File(CoreConfig.DAT_PATH + "/mobs/" + id + ".dat");
        if (!file.exists()) {
            return null;
        }
...
//            mts.addStatOptions(PImmune, pImmuneOpt);
    ...
    public static void loadMobsFromWz() {
        String wzDir1 = CoreConfig.WZ_PATH + "/Mob.wz";
        String wzDir2 = CoreConfig.WZ_PATH + "/Mob2.wz";
                            ...
                            for (Node skillInfoNode : XMLApi.getAllChildren(skillIDNode)) {
                                ...
                                switch (skillNodeName) {
                                    ...
                                    case "speak":
                                        ...
                                        break;
                                    default:
                                        log.warn(String.format("Unknown skills node %s with value %s", skillNodeName, skillNodeValue));
                                }
                            }
