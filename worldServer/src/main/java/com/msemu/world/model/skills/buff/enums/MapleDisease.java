...
public enum MapleDisease implements Serializable, IBuffStat {
    ...
    緩慢(MapleBuffStat.HiddenPieceOn, 126),
    ...
    影子(MapleBuffStat.StopMotion, 135), //receiving damage/moving
    ...
    旗幟(MapleBuffStat.IncMaxMP, 799); // PVP - Capture the Flag
    // 127 = 1 snow?
    // 129 = turn?
    ...
    public static MapleDisease getRandom() {
        while (true) {
            for (MapleDisease dis : MapleDisease.values()) {
                if (Rand.getChance(1.0 / MapleDisease.values().length)) {
                    return dis;
                }
            }
        }
