...
public class NetworkConfig {
    ...
    @ConfigComments(
            comment = {"Server version supported", "TWMS 202"}
    )
    @ConfigProperty(
            name = "network.server.version",
            value = "202"
    )
    public static int SERVER_VERSION;
    @ConfigComments(
            comment = {"Server patch version supported", "TWMS 202:1"}
    )
    @ConfigProperty(
            name = "network.server.patch",
            value = "1"
    )
    public static String SERVER_PATCH;
    ...
    @ConfigComments(
            comment = {"Show debug messages.", "Default: false"}
    )
    @ConfigProperty(
            name = "network.debug",
            value = "false"
    ...
    @ConfigComments(
            comment = {"IO Network thread execution mode.", "POOLED - All IO operations are executed in a special thread IO execution pool", "FIXED - All IO operations execution is spread across fixed number of treads", "Default: POOLED"}
    )
    @ConfigProperty(
