 ...
 */
public class MapleAESOFB implements ICipher {
    protected static final Logger log = LoggerFactory.getLogger(MapleAESOFB.class);

    protected byte[] iv;
    protected final short versionMask;
    protected final SecretKeySpec secretKeySpec;
    protected Cipher cipher;

    protected static final byte[] shiftKey = new byte[]{(byte) 0xEC, (byte) 0x3F, (byte) 0x77, (byte) 0xA4, (byte) 0x45, (byte) 0xD0, (byte) 0x71, (byte) 0xBF, (byte) 0xB7, (byte) 0x98, (byte) 0x20, (byte) 0xFC,
    ...
    public MapleAESOFB(final byte[] iv, short serverVersion, boolean isSend) {
        this.iv = iv;
        this.versionMask = isSend ? (short)(0xFFFF - serverVersion) : serverVersion;
        byte[] secretKey = MapleEncryptionKey.getEncryptionKey(serverVersion);
        ...
        try {
            ...
            log.error("MapleAESOFB initialized error", ex);
        }
    }

    public MapleAESOFB(int serverVersion) {
        this.iv = new byte[4];
        Rand.nextBytes(iv);
        this.versionMask = (short) (((serverVersion >> 8) & 0xFF) | ((serverVersion << 8) & 0xFF00));
        byte[] secretKey = MapleEncryptionKey.getEncryptionKey(serverVersion);
        this.secretKeySpec = new SecretKeySpec(secretKey, "AES");
        try {
            this.cipher = Cipher.getInstance("AES");
    ...
    @Override
    public byte[] encodeHeaderSize(int length) {
        int iiv = (((iv[3]) & 0xFF) | ((iv[2] << 8) & 0xFF00)) ^ versionMask;
    ...
    @Override
    public int decodeHeaderSize(int header) {
        int packetLength = ((header >>> 16) ^ (header & 0xFFFF));
        packetLength = ((packetLength << 8) & 0xFF00) | ((packetLength >>> 8) & 0xFF); // fix endianness
        return packetLength;
    }

    @Override
    public boolean checkHeader(int header) {
        return checkHeader(new byte[]{(byte) ((header >> 24) & 0xFF), (byte) ((header >> 16) & 0xFF)});
    }

    public boolean checkHeader(byte[] packet) {
        return ((((packet[0] ^ iv[2]) & 0xFF) == ((versionMask >> 8) & 0xFF)) && (((packet[1] ^ iv[3]) & 0xFF) == (versionMask & 0xFF)));
    }

    protected void updateIv() {
    ...
    public static void morph(byte inputByte, byte[] in) {
        byte a = in[1];
        byte b = inputByte;
        byte c = shiftKey[(int) a & 0xFF];
        c -= inputByte;
        in[0] += c;
        c = in[2];
        c ^= shiftKey[(int) b & 0xFF];
        a -= (int) c & 0xFF;
        in[1] = a;
        a = in[3];
        c = a;
        a -= (int) in[0] & 0xFF;
        c = shiftKey[(int) c & 0xFF];
        c += inputByte;
        c ^= in[2];
        in[2] = c;
        a += (int) shiftKey[(int) b & 0xFF] & 0xFF;
        in[3] = a;
    ...
    public boolean crypt(ByteBuffer buff, int offset, int length) {
        ...
        int procLength = 0x5B0;
        int startIndex = offset;
        try {
            while (remaining > 0) {
                byte[] myIv = BitUtils.multiplyBytes(this.iv, 4, 4);
                if (remaining < procLength) {
                    procLength = remaining;
                }
                for (int x = startIndex; x < (startIndex + procLength); x++) {
                    if ((x - startIndex) % myIv.length == 0) {
                        byte[] newIv = cipher.doFinal(myIv);
                        System.arraycopy(newIv, 0, myIv, 0, myIv.length);
                    }
                    buff.put(x, (byte) (buff.get(x) ^ myIv[(x - startIndex) % myIv.length]));
                }
                startIndex += procLength;
                remaining -= procLength;
                procLength = 0x5B4;
            }
            updateIv();
            return true;
        } catch (Exception except) {
            log.error("MapleAESOFB crypt error", except);
        }
        return false;
    }
}
